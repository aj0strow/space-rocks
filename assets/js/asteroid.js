(function(){
  ...
  var sizes = [ 1, 3, 6];
     ...
     *as its parent, but changes the angle by a passed value dthetha. It also changes how big the asteroid is*/
      ...
      this.position = { 
        x: parent.position.x + (dtheta / Math.abs(dtheta)), 
        y: parent.position.y + (dtheta / Math.abs(dtheta)) 
      };
  ...
  Asteroid.prototype = {
    ...
    collision: function() {
      var center = this.asteroidCenter,
          radius = this.asteroidRadius;
      
      return function(bullet) {
        return distance(bullet.position, center) < radius;
      };
    }
  }
